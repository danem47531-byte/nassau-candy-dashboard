...
uploaded_file = st.file_uploader("Upload your CSV data file to begin", type=["csv"])

if uploaded_file is None:
    st.info("👆 Please upload the Nassau Candy CSV file to load the dashboard.")
    st.stop()
...
k5.metric("Avg Margin Volatility", f"{vol_val:.2f}%" if not pd.isna(vol_val) else "N/A")
st.divider()
